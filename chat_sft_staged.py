...
# Model loading
parser.add_argument("--model-tag", type=str, default=None, help="legacy model tag (unused when --input-tag is given)")
parser.add_argument("--model-step", type=int, default=None, help="legacy model step (unused when --input-step is given)")
parser.add_argument("--stage", choices=["midtrain", "sft"], required=True,
                    help="midtrain=MMLU+GSM8K from base; sft=SmolTalk from Stage-1 checkpoint")
parser.add_argument("--input-tag", type=str, default=None, help="checkpoint tag to load")
parser.add_argument("--input-step", type=int, default=None, help="checkpoint step to load (default: latest)")
parser.add_argument("--output-tag", type=str, required=True, help="checkpoint tag to save under chatsft_checkpoints/")
parser.add_argument("--load-optimizer", type=int, default=1, help="warm-start optimizer from input checkpoint (0=no, 1=yes)")
# Training horizon
...
# Stage 1 loads from base_checkpoints; Stage 2 loads from chatsft_checkpoints.
input_source = "base" if args.stage == "midtrain" else "sft"
input_tag = args.input_tag if args.input_tag is not None else args.model_tag
input_step = args.input_step if args.input_step is not None else args.model_step
print0(f"Stage: {args.stage}")
print0(f"NANOCHAT_BASE_DIR: {get_base_dir()}")
print0(f"Loading checkpoint: source={input_source}, tag={input_tag}, step={input_step or 'latest'}")
print0(f"Saving checkpoint tag: {args.output_tag}")
model, tokenizer, meta = load_model(
    input_source, device, phase="train", model_tag=input_tag, step=input_step
)
...
if args.load_optimizer:
    optimizer_data = load_optimizer_state(input_source, device, rank=ddp_rank, model_tag=input_tag, step=input_step)
...
# Assignment stages: Stage 1=MMLU+GSM8K only; Stage 2=SmolTalk only.
...
print0(f"Training stage '{args.stage}': {len(train_dataset):,} rows")
print0(f"Validation rows: {len(val_dataset):,}")
...
while True:
    ...
    if last_step:
        output_dirname = args.output_tag
