...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Mid-train or supervised fine-tune the model")
# Logging
parser.add_argument("--run", type=str, default="dummy", help="wandb run name ('dummy' disables wandb logging)")
# Runtime
parser.add_argument("--device-type", type=str, default="", help="cuda|cpu|mps (empty = autodetect)")
# Model loading
parser.add_argument("--stage", type=str, default="midtrain", choices=["midtrain", "sft"], help="midtrain loads a base checkpoint; sft loads a Stage 1 checkpoint")
parser.add_argument("--model-tag", type=str, default=None, help="input checkpoint tag to load")
parser.add_argument("--model-step", type=int, default=None, help="model step to load from")
parser.add_argument("--output-tag", type=str, default=None, help="output checkpoint tag (default: reuse --model-tag)")
...
# Stage 1 starts from the pretrained base model. Stage 2 starts from the Stage 1
# checkpoint saved under chatsft_checkpoints.
input_source = "base" if args.stage == "midtrain" else "sft"
print0(f"Training stage: {args.stage} | Input checkpoint source: {input_source}")
model, tokenizer, meta = load_model(input_source, device, phase="train", model_tag=args.model_tag, step=args.model_step)
...
if args.load_optimizer:
    optimizer_data = load_optimizer_state(input_source, device, rank=ddp_rank, model_tag=args.model_tag, step=args.model_step)
...
# Keep reasoning adaptation and instruction following in separate stages.
if args.stage == "midtrain":
    ...
    train_description = f"MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs}"
    val_description = "MMLU + GSM8K"
else:
    train_tasks = [SmolTalk(split="train")]
    val_tasks = [SmolTalk(split="test")]
    train_description = "SmolTalk only"
    val_description = "SmolTalk only"

train_dataset = TaskMixture(train_tasks)
val_dataset = TaskMixture(val_tasks)
print0(f"Training mixture: {len(train_dataset):,} rows ({train_description})")
print0(f"Validation mixture: {len(val_dataset):,} rows ({val_description})")
