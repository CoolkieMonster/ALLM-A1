...
# Optional local structured logging. A fresh run replaces an older metrics file,
# while a resumed run appends to it.
metrics_jsonl_path = os.path.abspath(os.path.expanduser(args.metrics_jsonl)) if args.metrics_jsonl else None
if master_process and metrics_jsonl_path:
    os.makedirs(os.path.dirname(metrics_jsonl_path) or ".", exist_ok=True)
    if args.resume_from_step == -1:
        with open(metrics_jsonl_path, "w", encoding="utf-8"):
            pass
    print0(f"Structured metrics: {metrics_jsonl_path}")

def append_metric(record):
    """Append one JSON object on the master process only."""
    if not master_process or metrics_jsonl_path is None:
        return
...
# ----
# 在 [gpt.py](/nanochat/nanochat/gpt.py) 中：
...
# self.n_head = 1
# 所以：
# self.head_dim = 128 // 1 = 128
#创建 Query 线性层
#真正创建 Query 线性层的代码位于 [gpt.py](/nanochat/nanochat/gpt.py)：
# self.c_q = Linear(
#    self.n_embd,
...
# out_features = self.n_head * self.head_dim
#对于你的 depth-2 模型：
# in_features  = 128
# out_features = 1 × 128 = 128
# 因此相当于：
...
while True:
    ...
    synchronize()
    t0 = time.time()
    if args.log_train_bpb:
        step_train_nats = torch.zeros((), dtype=torch.float32, device=device)
        step_train_bytes = torch.zeros((), dtype=torch.int64, device=device)
    for micro_step in range(grad_accum_steps):
        if args.log_train_bpb:
            # Keep the ordinary token-mean cross-entropy for optimization, but
            # also accumulate nats and represented bytes for a tokenizer-neutral
            # training BPB metric. Special tokens have byte length zero and are
            # excluded from BPB, just as in evaluate_bpb().
            loss_per_token = model(x, y, loss_reduction="none").view(-1)
            targets = y.view(-1)
            valid = targets >= 0
            loss = loss_per_token.sum() / valid.sum().clamp_min(1)
            with torch.no_grad():
                safe_targets = torch.where(valid, targets, torch.zeros_like(targets))
                bytes_per_target = torch.where(
                    valid,
                    token_bytes[safe_targets],
                    torch.zeros_like(targets, dtype=token_bytes.dtype),
                )
                step_train_nats += (loss_per_token.detach() * (bytes_per_target > 0)).sum()
                step_train_bytes += bytes_per_target.sum()
        else:
            loss = model(x, y)
    ...
    if args.log_train_bpb:
        if is_ddp_initialized():
            dist.all_reduce(step_train_nats, op=dist.ReduceOp.SUM)
            dist.all_reduce(step_train_bytes, op=dist.ReduceOp.SUM)
        train_nats_f = step_train_nats.item()
        train_bytes_i = step_train_bytes.item()
        train_bpb_f = train_nats_f / (math.log(2) * train_bytes_i) if train_bytes_i else float("inf")
    synchronize()
    t1 = time.time()
    dt = t1 - t0
    # -------------------------------------------------------------------------
    ...
    train_bpb_str = f" | train_bpb: {train_bpb_f:.6f}" if train_bpb_f is not None else ""
    print0(f"step {step:05d}/{num_iterations:05d} ({pct_done:.2f}%) | loss: {debiased_smooth_loss:.6f}{train_bpb_str} | lrm: {lrm:.2f} | dt: {dt * 1000:.2f}ms | tok/sec: {tok_per_sec:,} | bf16_mfu: {mfu:.2f} | epoch: {epoch} | total time: {total_training_time/60:.2f}m{eta_str}")
    if train_bpb_f is not None:
        append_metric({
            "record": "metric",
            "split": "train",
            "step": step,
